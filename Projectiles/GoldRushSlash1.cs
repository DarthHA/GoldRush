...
{
    public class GoldRushSlash1 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            Vector2 OffSet = new Vector2(7, -10);
            ...
            {
                Projectile.Center += (Projectile.ai[0] / 4f * 16f - 5) * rot.ToRotationVector2();
            }
            else
            {
                Projectile.Center += 11f * rot.ToRotationVector2();
            }
            if (Projectile.ai[0] == 1)
            {
                ...
                Projectile.NewProjectile(null,owner.Center, Vector2.Zero, ModContent.ProjectileType<GoldRushSlashGlow1>(), Projectile.damage, 0, owner.whoAmI);
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (Projectile.ai[0] <= 4)
            {
                rot += -MathHelper.Pi / 3 * owner.direction * Projectile.ai[0] / 5f;
            }
            else
            {
                rot += -MathHelper.Pi / 3 * owner.direction;
            }
            if (owner.gravDir < 0)
            {
                rot = new Vector2(rot.ToRotationVector2().X, -rot.ToRotationVector2().Y).ToRotation();
            }
            Main.spriteBatch.Draw(tex, Projectile.Center - Main.screenPosition, null, lightColor * (1 - owner.immuneAlpha / 255f), rot, tex.Size() / 2, Projectile.scale * 0.4f, SP, 0);

            foreach (Projectile glow in Main.projectile)
            {
                if (glow.active && glow.type == ModContent.ProjectileType<GoldRushSlashGlow1>() && glow.owner == Projectile.owner)
                {
                    (glow.ModProjectile as GoldRushSlashGlow1).DrawAlt(Main.spriteBatch);
                }
            }
            return false;
        }
